...
Move the call out of the ``steady_state(…)`` operator.",
    ...
    ("E432", ExplainEntry {
        title: "var(log) variable in a VAR, trend, or PAC equation",
        body: "A selected VAR or trend-component equation, or an equation with pac_expectation, uses an endogenous declared with var(log). Dynare refuses at transform: `the following variables are declared with var(log) and therefore cannot appear in a VAR/TCM/PAC equation`.\n\n**Fix**\n\nRemove the log declaration for that variable or change the selected equation.",
        kind: ExplainKind::Shared,
    }),
    ("E433", ExplainEntry {
        title: "Selected model equation tag is absent",
        body: "A var_model or trend_component_model eqtags/targets list names no equation in the effective written model. Dynare refuses at transform: `looking for equation tag X failed.` or `no equation is named 'X'`. model_remove and model_replace affect which tags exist.\n\n**Fix**\n\nUse a present [name=] tag or restore the removed equation.",
        kind: ExplainKind::Shared,
    }),
    ("E434", ExplainEntry {
    ...
    ("E435", ExplainEntry {
        title: "VAR expectation auxiliary model is absent",
        body: "A var_expectation_model refers to an auxiliary_model_name that is neither a var_model nor a trend_component_model. Dynare refuses at transform: `var_expectation_model NAME refers to nonexistent auxiliary model AUX`.\n\n**Fix**\n\nDeclare the named auxiliary model or correct the name.",
        kind: ExplainKind::Shared,
    }),
    ("E436", ExplainEntry {
    ...
    ("E437", ExplainEntry {
        title: "PAC options conflict with target information",
        body: "A pac_model gives growth, auxname, or kind while a matching pac_target_info block exists, or gives kind for a model-consistent-expectations PAC. Dynare refuses at check with the option-specific `for PAC model ... it is not possible ...` sentence.\n\n**Fix**\n\nPut target components in pac_target_info and remove the conflicting pac_model option.",
        kind: ExplainKind::Shared,
    }),
    ("E438", ExplainEntry {
    ...
    ("E440", ExplainEntry {
        title: "Semi-structural model name is repeated",
        body: "A second command of the same semi-structural model kind uses an earlier model_name. Dynare refuses while parsing: `a VAR model already exists with the name NAME`, with corresponding wording for the other three kinds.\n\n**Fix**\n\nGive the second model a distinct name or remove it.",
        kind: ExplainKind::Shared,
    }),
    ("E441", ExplainEntry {
    ...
    ("E442", ExplainEntry {
        title: "VAR expectation discount has the wrong type",
        body: "A var_expectation_model discount is neither a constant expression nor a parameter. Dynare refuses while parsing: `The discount factor must be a constant expression or a parameter`.\n\n**Fix**\n\nUse a numeric constant or declared parameter.",
        kind: ExplainKind::Shared,
    }),
    ("E443", ExplainEntry {
        title: "VAR expectation time shift is positive",
        body: "Dynare refuses a positive var_expectation_model time_shift while parsing: `The 'time_shift' option must be a non-positive integer`.\n\n**Fix**\n\nUse zero or a negative integer.",
        kind: ExplainKind::Shared,
    }),
    ("E444", ExplainEntry {
        title: "PAC discount is not a parameter",
        body: "The pac_model discount names a declared non-parameter symbol. Dynare refuses while parsing: `NAME is not a parameter`.\n\n**Fix**\n\nUse a declared parameter.",
        kind: ExplainKind::Shared,
    }),
    ("E445", ExplainEntry {
        title: "Named VAR or PAC operator inside epilogue",
        body: "Dynare refuses var_expectation, pac_expectation, and pac_target_nonstationary inside epilogue while parsing: `The 'OPERATOR' operator is forbidden in 'epilogue'.` The model block can contain these operators.\n\n**Fix**\n\nRemove the operator from epilogue or move the calculation into a model equation.",
        kind: ExplainKind::Shared,
    }),
    ("E446", ExplainEntry {
        title: "PAC auxiliary model name is absent",
        body: "A pac_model auxiliary_model_name names no var_model or trend_component_model. Dynare refuses at transform: `aux_model_name not recognized as VAR model or Trend Component model`.\n\n**Fix**\n\nDeclare the named auxiliary model or correct the name.",
        kind: ExplainKind::Shared,
    }),
    ("E447", ExplainEntry {
        title: "VAR expectation operator names no model",
        body: "A model equation uses var_expectation(NAME) without a matching var_expectation_model. Dynare refuses at transform: `unknown model 'NAME' used in var_expectation expression`.\n\n**Fix**\n\nDeclare a var_expectation_model with that model_name or correct the call.",
        kind: ExplainKind::Shared,
    }),
    ("E448", ExplainEntry {
    ...
    ("E449", ExplainEntry {
        title: "PAC model has no expectation equation",
        body: "A pac_model has no effective written equation using its pac_expectation(NAME) operator. Dynare refuses at transform: `the model does not contain the 'pac_expectation(NAME)' operator.`\n\n**Fix**\n\nUse the named operator in a model equation, or remove the unused pac_model.",
        kind: ExplainKind::Shared,
    }),
    ("E450", ExplainEntry {
        title: "PAC expectation appears in several equations",
        body: "Two effective written equations use pac_expectation(NAME) for the same PAC model. Dynare refuses at transform: `It is not possible to use 'pac_expectation(NAME)' in several equations.` Two calls in one equation are accepted.\n\n**Fix**\n\nKeep that PAC expectation in one equation.",
        kind: ExplainKind::Shared,
    }),
    ("E451", ExplainEntry {
        title: "PAC expectation names no PAC model",
        body: "A written model equation uses pac_expectation(NAME) without a matching pac_model. Dynare refuses at transform: `the pac_expectation operator references an unknown pac_model`.\n\n**Fix**\n\nDeclare the named pac_model or correct the operator name.",
        kind: ExplainKind::Shared,
    }),
    ("E452", ExplainEntry {
    ...
    ("E453", ExplainEntry {
        title: "PAC growth correction name conflicts with a declaration",
        body: "A written declaration uses the name NAME_pac_growth_neutrality_correction generated by a pac_model growth option. Dynare refuses at transform with its growth-neutrality correction conflict sentence.\n\n**Fix**\n\nRename the declared symbol.",
        kind: ExplainKind::Shared,
    }),
    ("E454", ExplainEntry {
        title: "MCE PAC parameter name conflicts with a declaration",
        body: "A written declaration uses the first mce_alpha_NAME_1 parameter generated for a model-consistent-expectations PAC. Dynare refuses at transform with its generated-parameter conflict sentence.\n\n**Fix**\n\nRename the declared symbol.",
        kind: ExplainKind::Shared,
    }),
    ("E455", ExplainEntry {
        title: "Backward or component PAC parameter name conflicts",
        body: "A written declaration uses a fixed backward-VAR constant or component growth-correction parameter name generated by a PAC model. Dynare refuses at transform with its auxiliary-parameter conflict sentence. Lag-indexed names need the rewritten model and remain silent.\n\n**Fix**\n\nRename the declared symbol.",
        kind: ExplainKind::Shared,
    }),
    ("E456", ExplainEntry {
        title: "PAC expectation auxiliary name conflicts",
        body: "A written declaration uses an auxiliary variable name generated for pac_expectation, or a PAC component/model auxname reuses a target or expectation auxiliary name generated earlier, including by another PAC model. Dynare refuses at transform with its generated-variable conflict sentence.\n\n**Fix**\n\nGive each auxiliary a distinct name and keep those names free of declared symbols.",
        kind: ExplainKind::Shared,
    }),
    ("E457", ExplainEntry {
        title: "PAC nonstationary target auxiliary name conflicts",
        body: "A written declaration or previously generated PAC auxiliary uses the auxname_target_nonstationary of a PAC target-info block. Dynare refuses at transform with its pac_target_nonstationary generated-variable conflict sentence.\n\n**Fix**\n\nGive the nonstationary target a distinct auxiliary name.",
        kind: ExplainKind::Shared,
    }),
    ("E458", ExplainEntry {
        title: "PAC target expression has no defining equation",
        body: "A direct target product such as v*v has no matching left-hand side in the effective written model. Dynare refuses at transform: `there is no equation whose LHS is equal to the 'target' of 'pac_target_info(NAME)'`. Other target expressions may depend on rewrite and stay silent.\n\n**Fix**\n\nMake target match an equation left-hand side, or use the intended target variable.",
        kind: ExplainKind::Shared,
    }),
    ("W206", ExplainEntry {
